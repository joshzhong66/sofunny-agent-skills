...
const DEFAULT_SERVICE_URL = process.env.SEEDANCE_STUDIO_SERVICE_URL || 'http://10.20.3.69:3001';
const DEFAULT_POLL_INTERVAL_MS = Number(process.env.SEEDANCE_STUDIO_POLL_INTERVAL_MS || 3000);
const DEFAULT_TIMEOUT_SECONDS = Number(process.env.SEEDANCE_STUDIO_TIMEOUT_SECONDS || 600);
const DEFAULT_DOWNLOAD_DIR = process.env.SEEDANCE_STUDIO_DOWNLOAD_DIR || 'seedance-downloads';
const DEFAULT_DURATION = 5;
const DEFAULT_ASPECT_RATIO = '16:9';
const DEFAULT_RESOLUTION = '1080p';
const SUCCESS_STATES = new Set(['COMPLETED', 'SUCCESS', 'SUCCEEDED', 'DONE', 'COMPLETED']);
...
async function saveProjectCopy(config, result) {
  ...
  const modelPart = sanitizeFilenamePart(
    result.historyRecord?.model || config.model || 'seedance',
    'seedance'
  );
...
async function run() {
  ...
  const rawConfig = await promptForMissingFields({
    apiKey: firstNonEmpty(args['api-key'], process.env.SEEDANCE_STUDIO_API_KEY),
    prompt: firstNonEmpty(args.prompt, process.env.SEEDANCE_STUDIO_PROMPT),
    serviceUrl: firstNonEmpty(
      args['service-url'],
      process.env.SEEDANCE_STUDIO_SERVICE_URL,
      DEFAULT_SERVICE_URL
    ),
    apiUrl: firstNonEmpty(
      args['api-url'],
      process.env.SEEDANCE_STUDIO_API_URL,
      process.env.NEWAPI_BASE_URL
    ),
    model: firstNonEmpty(
      args.model,
      process.env.SEEDANCE_STUDIO_MODEL,
      process.env.SEEDANCE_MODEL
    ),
    duration: firstNonEmpty(args.duration, process.env.SEEDANCE_STUDIO_DURATION),
    aspectRatio: firstNonEmpty(args['aspect-ratio'], process.env.SEEDANCE_STUDIO_ASPECT_RATIO),
    resolution: firstNonEmpty(args.resolution, process.env.SEEDANCE_STUDIO_RESOLUTION),
    imageUrl: firstNonEmpty(args['image-url'], process.env.SEEDANCE_STUDIO_IMAGE_URL),
    videoUrl: firstNonEmpty(args['video-url'], process.env.SEEDANCE_STUDIO_VIDEO_URL),
    downloadDir: firstNonEmpty(
      args['download-dir'],
      process.env.SEEDANCE_STUDIO_DOWNLOAD_DIR,
      DEFAULT_DOWNLOAD_DIR
    ),
    skipProjectDownload: Boolean(args['skip-project-download']) ||
      parseBooleanEnv(process.env.SEEDANCE_STUDIO_SKIP_PROJECT_DOWNLOAD),
    timeoutSeconds: firstNonEmpty(
      args['timeout-seconds'],
      process.env.SEEDANCE_STUDIO_TIMEOUT_SECONDS
    ),
    pollIntervalMs: firstNonEmpty(
      args['poll-interval-ms'],
      process.env.SEEDANCE_STUDIO_POLL_INTERVAL_MS
    )
  });
  ...
  if (!config.apiKey) {
    throw new Error(
      'Missing api key. Pass --api-key, set SEEDANCE_STUDIO_API_KEY, or run in an interactive terminal.'
    );
  }

  if (!config.prompt) {
    throw new Error(
      'Missing prompt. Pass --prompt, set SEEDANCE_STUDIO_PROMPT, or run in an interactive terminal.'
    );
  }
